...
- `/wawo` - the Django project folder
	...
	- `wawo` - Project settings, asgi settings, and base urls
...
#### DJANGO
- make sure you are still in the environment, otherwise run `workon wawo`
...
- Pull Remote Database (to replace the local wawo database with one of the live databases):
...
- Django Shell on Remote Server ***<span style="color:red;">(caution: this can potentially change/delete production data. Use with a healthy amount of caution and capture a database backup first (see below for instructions))</span>***:
    - Install and log into the the Heroku CLI:
        - For Mac: use Homebrew: `brew tap heroku/brew && brew install heroku` from <https://devcenter.heroku.com/articles/heroku-cli>
    - open a bash shell on the remote server: run: `heroku run bash --app [app name such as wawo-dev]`
    - once in the shell on the Heroku server run: `python3.8 manage.py shell_plus`
- Capture a Remote Database Backup:
    - run: `heroku pg:backups:capture --app [app name such as wawo-dev]`
...
- Retrieve Media assets from AWS S3:
    - Set up and configure the AWS CLI: see <https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-install.html> for instructions
        - be sure to set up your AWS config with your wawo credentials: instructions here <https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html>
    - make sure a directory exists with the path [project root]/wawo/media
    - from the project root run: `aws s3 cp s3://wawo-dev ./wawo/media --recursive` replacing `wawo-dev` with `wawo-prod` or a different s3 bucket name as needed
